...
const toggleDashboard = () => {
  ...
  if (isOpen.value) {
      tempDailyGoal.value = dailyGoal.value;
      tempProjectTarget.value = projectTarget.value;
  }
};
      ...
      >
        ...
        >
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                <button @click="isOpen = false" class="text-ink/40 hover:text-ink">
            ...
            <div class="space-y-6">
                ...
                <!-- Daily Goal -->
                <div class="space-y-2">
                    <div class="flex justify-between text-xs uppercase tracking-widest font-bold text-ink/50">
                        <span>Daily Goal</span>
                        <span v-if="!isEditing">{{ Math.round(progressDaily) }}%</span>
                    </div>
                    
                    <div v-if="isEditing" class="flex gap-2">
                         <input v-model="tempDailyGoal" type="number" class="w-full bg-stone/10 rounded px-2 py-1 text-sm border-none focus:ring-1 focus:ring-accent" />
                    </div>
                    <div v-else class="relative h-2 bg-stone/10 rounded-full overflow-hidden">
                        <div class="absolute top-0 left-0 h-full bg-accent transition-all duration-700 ease-out" :style="{ width: `${progressDaily}%` }"></div>
                    </div>
                    <div v-if="!isEditing" class="text-xs font-mono text-ink/70 text-right">{{ todayWords }} / {{ dailyGoal }} words</div>
                </div>
                ...
                <div class="space-y-2">
                    <div class="flex justify-between text-xs uppercase tracking-widest font-bold text-ink/50">
                     ...
                     <div v-if="isEditing" class="flex gap-2">
                         <input v-model="tempProjectTarget" type="number" class="w-full bg-stone/10 rounded px-2 py-1 text-sm border-none focus:ring-1 focus:ring-accent" />
                    </div>
                    <div v-else class="relative h-2 bg-stone/10 rounded-full overflow-hidden">
                        <div class="absolute top-0 left-0 h-full bg-ink transition-all duration-700 ease-out" :style="{ width: `${progressProject}%` }"></div>
                    </div>
                     <div v-if="!isEditing" class="text-xs font-mono text-ink/70 text-right">{{ totalProjectWords }} / {{ projectTarget }} words</div>
                </div>

                <!-- History Chart -->
                <div class="pt-4 border-t border-stone/10">
                    <div class="flex justify-between text-xs uppercase tracking-widest font-bold text-ink/50 mb-3">
                        <span>Last 7 Days</span>
                    </div>
                    <div class="flex items-end justify-between h-24 gap-1">
                        <div v-for="day in chartData" :key="day.date" class="flex-1 flex flex-col items-center gap-1 group/bar relative">
                            <!-- Tooltip -->
                             <div class="absolute bottom-full mb-1 bg-ink text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover/bar:opacity-100 transition-opacity whitespace-nowrap z-10 pointer-events-none">
                                {{ day.count }} words
                            </div>
                            ...
                            >
                                <div class="absolute bottom-0 w-full bg-accent/40" :style="{ height: day.count > 0 ? '4px' : '0' }"></div>
                            </div>
                            <!-- Label -->
