...
<template>
  <div class="space-y-8">
      ...
      <div class="space-y-2">
        ...
        <div class="grid grid-cols-3 gap-2">
          <button
            ...
            :class="
              settings.editor.fontFamily === 'serif'
                ? 'bg-accent/10 border-accent shadow-[0_0_15px_rgba(255,95,31,0.1)]'
                : 'bg-white/70 dark:bg-white/5 border-white dark:border-white/10 hover:border-accent/40 shadow-sm'
            "
            @click="settings.editor.fontFamily = 'serif'"
          >
            <span
              class="font-serif text-xl relative z-10"
              :class="settings.editor.fontFamily === 'serif' ? 'text-accent' : 'text-ink/70'"
              >Ag</span
            >
            <div
              class="text-[9px] font-bold mt-1 uppercase tracking-widest relative z-10"
              :class="settings.editor.fontFamily === 'serif' ? 'text-accent/60' : 'text-ink/30'"
            >
              Serif
            </div>
          </button>
          ...
          >
            <span
              class="font-sans text-xl relative z-10"
              :class="settings.editor.fontFamily === 'sans' ? 'text-accent' : 'text-ink/70'"
              >Ag</span
            >
            <div
              class="text-[9px] font-bold mt-1 uppercase tracking-widest relative z-10"
              :class="settings.editor.fontFamily === 'sans' ? 'text-accent/60' : 'text-ink/30'"
            >
              Sans
            </div>
          </button>
          <button
            class="px-4 py-4 border rounded-xl text-center transition-all group relative overflow-hidden active:scale-95"
            :class="
              settings.editor.fontFamily === 'mono'
                ? 'bg-accent/10 border-accent shadow-[0_0_15px_rgba(255,95,31,0.1)]'
                : 'bg-white/70 dark:bg-white/5 border-white dark:border-white/10 hover:border-accent/40 shadow-sm'
            "
          ...
          >
            <span
              class="font-mono text-xl relative z-10"
              :class="settings.editor.fontFamily === 'mono' ? 'text-accent' : 'text-ink/70'"
              >Ag</span
            >
            <div
              class="text-[9px] font-bold mt-1 uppercase tracking-widest relative z-10"
              :class="settings.editor.fontFamily === 'mono' ? 'text-accent/60' : 'text-ink/30'"
            >
              Mono
            </div>
          </button>
        ...
        <div class="space-y-3">
          <div class="flex justify-between items-end">
            ...
            <span class="text-[10px] font-mono text-accent font-bold"
              >{{ settings.editor.fontSize }}px</span
            >
          </div>
          <input
    ...
    <div class="space-y-4">
      ...
      <div
        class="flex items-center justify-between p-4 bg-white/70 dark:bg-white/5 border border-white dark:border-white/10 rounded-xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]"
      >
        ...
        <button
          ...
          :class="
            settings.editor.focusMode
              ? 'bg-accent shadow-[0_0_10px_rgba(255,95,31,0.4)]'
